...
public class SoundScreen extends GameAssets implements Screen, InputProcessor {
	...
	public SoundScreen(JetAdventure game, ActionResolver actionResolver) {
	...
	public void addButton(Texture buttonTextures, float posX, float posY,
		...
		button.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				if (action.equals(GameCode.GAME_MENU)) {
					gameAlpha = 1.0f;
	...
	private void drawSoundOptions() {
		...
		for (int i = 0; i < 4; i++) {
			spriteBatch.draw(volumeProgressTextureRegion[i],
					volumeControllProgressX[i], move
							+ volumeControllProgressY[i],
					volumeControllProgressW[i], volumeControllProgressH[i]);

			spriteBatch.draw(volumeIconTextureRegion[i],
					volumeControllIconX[i], move + volumeControllIconY[i],
					volumeControllIconW[i], volumeControllIconH[i]);
		...
		for (int i = 0; i < 4; i++) {
			float heightIncrement = (i % 2 == 0 ? 0 : height / 10);
			heightIncrement = 0;
			font2.draw(
					spriteBatch,
					tempSoundCaption[i],
					3 * width / 12 + i * width / 6
							- font.getBounds(tempSoundCaption[i]).width / 4,
					-move + height / 8 + heightIncrement);
			font2.draw(
					spriteBatch,
					tempVolumeValue[i],
					3 * width / 12 + i * width / 6
							- font.getBounds(tempVolumeValue[i]).width / 4,
					-move + height / 8 + 2
	...
	private void buttonInit() {
		...
		addButton(menuButtonTexture, move, 0, BUTTON_WIDTH, BUTTON_WIDTH,
				GameCode.GAME_MENU);
		addButton(quitButtonTexture, -move + width - BUTTON_WIDTH, 0,
				BUTTON_WIDTH, BUTTON_WIDTH, GameCode.QUIT);
	...
	private void drawSoundScreenBackground() {
		spriteBatch.draw(allBackgroundTextureRegion, 0, 0, width, height);
		spriteBatch.draw(screenOverTextureRegion, 0, 0, width, height);
		spriteBatch.draw(screenOverTextureRegion, 0, 0, width, height);
	}
